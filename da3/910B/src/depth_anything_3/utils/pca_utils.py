...
def pca_to_rgb_4d_bf16_percentile(
    ...
    - Uses NPU bf16 autocast in computation (if available),
    ...
    device : str | None
        Specify 'npu' or 'cpu'. Auto-select if None (prefer npu).
    q_oversample : int
    ...
    with torch.no_grad():
        ...
        device.startswith("npu") and enable_autocast_bf16
...
class PCARGBVisualizer:
    ...
    def __init__(
        ...
        self.device = device or ("npu" if torch.npu.is_available() else "cpu")
