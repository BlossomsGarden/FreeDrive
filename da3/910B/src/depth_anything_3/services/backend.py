...
# get_npu_memory_info imported from depth_anything_3.utils.memory


# cleanup_npu_memory imported from depth_anything_3.utils.memory
...
def create_app(model_dir: str, device: str = "npu", gallery_dir: Optional[str] = None) -> FastAPI:
...
<body>
    ...
    async def get_status():
        ...
        gpu_memory = get_npu_memory_info()
        if gpu_memory:
            status["gpu_memory"] = {
    ...
    async def get_gpu_memory():
        ...
        if gpu_memory is None:
            return {
                "available": False,
                "message": "NPU not available or memory info cannot be retrieved",
            }
...
if __name__ == "__main__":
    ...
    parser.add_argument("--device", default="npu", help="Device to use")
