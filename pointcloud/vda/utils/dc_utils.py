...
def save_video(frames, output_video_path, fps=10, is_depths=False, grayscale=False):
    ...
    try:
        writer = imageio.get_writer(output_video_path, fps=fps, macro_block_size=1, codec='libx264', ffmpeg_params=ffmpeg_params)
    except Exception as e:
        # Fallback: try without ffmpeg_params if the above fails
        print(f"Warning: Failed to use ffmpeg_params {ffmpeg_params}, trying without custom parameters: {e}")
        try:
            writer = imageio.get_writer(output_video_path, fps=fps, macro_block_size=1, codec='libx264')
        except Exception as e2:
            # Final fallback: use default codec
            print(f"Warning: Failed to use libx264 codec, trying default codec: {e2}")
